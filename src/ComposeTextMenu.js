...
function ComposeTextMenu() {
  ...
  return (
    <div>
      <button id="multiLevelDropdownTextButton" onClick={() => openDropdown()} onMouseDown={(e) => e.preventDefault()} data-dropdown-toggle="dropdownText" className="shadow rounded-lg px-4 py-2.5 mx-2.5 w-56 text-center inline-flex items-center justify-between focus:outline-none bg-white" type="button">
          ...
          {actions.map((action, index) => {
            if(action.popUpType == 2)
            {
              if(action.subActions == undefined){
                return (
                  <li key={index}>
                    <a onClick={() => executeAction(action)} onMouseDown={(e) => e.preventDefault()} className="block flex py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">
                      {action.icon}
                ...
                return (
                  <li key={index}>
                    <a id="multiLevelDropdownTextButton" onClick={() => openSecondDropdown(action.name, event)} onMouseDown={(e) => e.preventDefault()} className="block py-2 flex hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white justify-between pr-2 cursor-pointer">
      ...
      <div id="doubleDropdownText" className="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 fixed hidden">
        {
          actions.map((action, index) => {
                ...
                return (
                  <ul key={index} className="myUl py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="doubleDropdownTextButton">
                    {action.subActions.map((subAction, indexBis) => {
                      return (
                        <li key={indexBis}>
                          <a onClick={() => executeAction(action, subAction)} onMouseDown={(e) => e.preventDefault()} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">{subAction.label}</a>
                        </li>
                      )
                    })}
                  </ul>
