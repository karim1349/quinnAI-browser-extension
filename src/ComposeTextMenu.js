...
function ComposeTextMenu() {
  ...
  const openDropdown = () => {
    ...
    if (dropdown.classList.contains("hidden")) {
      ...
      if(document.getElementById("doubleDropdownText").classList.contains("hidden") == false){
        document.getElementById("doubleDropdownText").classList.add("hidden");
      }
    }
  }
  ...
  const openSecondDropdown = (id, event) => {
    ...
    const secondDropdownElement = document.getElementById("doubleDropdownText");
    const firstDropdownWidth = document.getElementById("dropdownText").offsetWidth;
    ...
    if(document.getElementById("doubleDropdownText").classList.contains("hidden") == true){
      document.getElementById("doubleDropdownText").classList.remove("hidden");
    } else if (oldId == id){
      document.getElementById("doubleDropdownText").classList.add("hidden");
    }
  }
  ...
  document.addEventListener("click", function (event) {
    const dropdown = document.getElementById("dropdownText");
    if(!dropdown) return;
    if (event.target.closest("#multiLevelDropdownTextButton")) return;
    document.getElementById("svgInset").classList.remove("rotate-180");
    document.getElementById("svgInset").classList.add("rotate-0");
    dropdown.classList.add("hidden");
    if(document.getElementById("doubleDropdownText").classList.contains("hidden") == false){
  ...
  return (
    <div>
      <button id="multiLevelDropdownTextButton" onClick={() => openDropdown()} onMouseDown={(e) => e.preventDefault()} data-dropdown-toggle="dropdownText" class="shadow rounded-lg px-4 py-2.5 mx-2.5 w-56 text-center inline-flex items-center justify-between focus:outline-none bg-white" type="button">
      ...
      <div id="dropdownText" class="z-10 hidden bg-white mx-2.5 my-2 h-40 rounded-lg shadow w-56 absolute overflow-y-scroll scrollableDropdown">
        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="multiLevelDropdownTextButton">
          {actions.map((action, index) => {
            if(action.popUpType == 2)
            {
              if(action.subActions == undefined){
                return (
                  <li>
                    <a onClick={() => executeAction(action)} onMouseDown={(e) => e.preventDefault()} class="block flex py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">
                      {action.icon}
                ...
                return (
                  <li>
                    <a id="multiLevelDropdownTextButton" onClick={() => openSecondDropdown(action.name, event)} onMouseDown={(e) => e.preventDefault()} class="block py-2 flex hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white justify-between pr-2 cursor-pointer">
      ...
      <div id="doubleDropdownText" class="z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 fixed hidden">
        {
          actions.map((action, index) => {
            if(action.name == secondDropdown && action.popUpType == 2){
              if(action.subActions != undefined){
                return (
                  <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="doubleDropdownTextButton">
                    {action.subActions.map((subAction, index) => {
                      return (
                        <li>
                          <a onClick={() => executeAction(action, subAction)} onMouseDown={(e) => e.preventDefault()} class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">{subAction.label}</a>
                        </li>
                      )
                    })}
                  </ul>
...
export default ComposeTextMenu;