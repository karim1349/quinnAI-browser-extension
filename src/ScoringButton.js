...
function ScoringButton() {
    ...
    const scoreEmails = () => {
        const emailList = window.gmail.dom.inbox_content()[0].getElementsByClassName("F cf zt")[0].childNodes[1];
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if(mutation.addedNodes.length > 1) {
                    ...
                    //If email hasn't .score in his children, add it
                    if(email.getElementsByClassName("score").length == 0) {
    ...
    return (
        <div className="flex justify-center">
            <button className="z-10 hover:bg-gray-100 rounded-full" style={{height:35}} onClick={() => scoreEmails()}>
                ...
                <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-10">
                    ...
                    <div className="inline-block align-bottom bg-white text-left shadow-xl transform transition-all sm:my-8 sm:align-middle w-3/5 rounded-lg" role="dialog" aria-modal="true" aria-labelledby="modal-title">
                            ...
                            <div>
                                ...
                                <div className="mt-3 p-4 text-center sm:mt-0 sm:text-left">
                                    ...
                                    <div className="mt-2 w-100">
                                        <div className="h-16 bg-cyan-700 w-100 bg-lightblue1 justify-between flex items-center pl-5 pr-5">
                                            Emails sélectionnés pour le scoring
                                            <input type={"checkbox"} className="ml-2" />
                                        </div>
                                        <div className="h-16 bg-cyan-700 w-100 bg-lightblue2 justify-between flex items-center pl-5 pr-5">
                                            adresse@email.com
                                            <input type={"checkbox"} className="ml-2" />
                                        </div>
                                        <div className="h-16 bg-cyan-700 w-100 bg-lightblue2 justify-between flex items-center pl-5 pr-5">
                        ...
                        <div className="flex items-center">
                            <button onClick={() => console.log("score")} type="button" className="w-full inline-flex justify-center rounded-md border-transparent px-4 py-2 buttonConfirm text-base font-medium text-white hover:bg-blue-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                                Scorer
                            </button>
                        </div>
                        </div>
