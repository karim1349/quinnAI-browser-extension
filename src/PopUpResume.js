...
function PopUpResume({ originalSubAction, email, action, setAction }) {
    const [subAction, setSubAction] = React.useState(originalSubAction);
    const [lastSubAction, setLastSubAction] = React.useState(originalSubAction);
    const [output, setOutput] = React.useState(0);
    const source = htmlToText(email.body());
    const openDropdown = () => {
    ...
    useEffect(() => {
        setOutput(0);
        action.function(source, subAction?.name)
        .then(data => {            
            setOutput(data);
        })
    }, [lastSubAction])
    return (
        <div className="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div onClick={() => setAction(0)} className="fixed inset-0 bg-gray-500 bg-opacity-25 transition-opacity" aria-hidden="true"></div>
                ...
                <div className="inline-block align-bottom bg-white text-left shadow-xl transform transition-all sm:my-8 sm:align-middle w-3/5 rounded-lg" role="dialog" aria-modal="true" aria-labelledby="modal-title">
                    <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 rounded-t-lg">
                        <div>
                            <div className="absolute right-2" onClick={() => setAction(0)}>
                            ...
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div className="mt-2 w-100">
                                    ...
                                    <h2 className="text-sm text-black" >
                                        ...
                                        {output == 0 ? 
                                            <div className="w-100 h-100 max-h-96 overflow-scroll inputArea mt-5 mb-5" id="resume" name="resume" readOnly>
                    ...
                    <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse justify-between rounded-b-lg">
                        <div className="flex items-center">
                            {
                                ...
                                null :
                                <div className="refreshButton" onClick={() => setLastSubAction(subAction)}>
                                    <RefreshIcon/>
                                </div>

                            }
                            <button onClick={() => setAction(0)} type="button" className="w-full inline-flex justify-center rounded-md border-transparent px-4 py-2 buttonConfirm text-base font-medium text-white hover:bg-blue-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                                Fermer
                            </button>
                        </div>
                        <div>
                        {
                            action.subActions &&
                            <>
                                <button id="multiLevelDropdownButton2" onClick={() => openDropdown()} data-dropdown-toggle="dropdown" className="shadow rounded-lg px-4 py-2.5 mx-2.5 w-48 text-center inline-flex items-center justify-between focus:outline-none" type="button">
                                    <div className="flex items-center">
                                    <svg width="32" height="15" viewBox="0 0 32 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M12.7119 2.36695V5.28869L0 6.25681H16.2714L12.7119 6.83652V8.15053L2.77762 8.90705H19.05L12.7119 9.7399V11.8993L3.40526 12.6075H19.6776L12.7119 13.3457C12.7119 14.0752 13.3029 14.6674 14.0308 14.6674H30.6811C31.409 14.6674 32 14.0752 32 13.3457V2.36695L22.3559 9.06164L12.7119 2.36695ZM32 0.754395H12.7119L22.3559 7.44908L32 0.754395Z" fill="#00ADEF"/>
                                    </svg>
                                    <span className="text-neutral-700 px-2 font-light text-sm whitespace-nowrap overflow-hidden w-24" style={{textOverflow:'ellipsis'}}>
                                        {subAction?.label}
                                    </span>
                                    </div>
                                ...
                                <div id="dropdown2" className="z-10 hidden bg-white mx-2.5 my-2 rounded-lg shadow w-48 absolute">
                                    <ul className="myUl py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="multiLevelDropdownButton2">
                                        {action.subActions.map((subAction, index) => {
                                            return(
                                                <li key={index}>
                                                    <a onClick={() => setSubAction(subAction)} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{subAction.label}</a>
                                                </li>
                                            )
                                        })}
                                    </ul>
